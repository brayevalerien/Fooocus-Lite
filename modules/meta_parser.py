...
def load_parameter_button_click(raw_metadata: dict | str, is_generating: bool):
    ...
    get_str(
        "refiner_swap_method", "Refiner Swap Method", loaded_parameter_dict, results
    )
    ...
    for i in range(modules.config.default_max_lora_number):
        get_lora(
            f"lora_combined_{i + 1}", f"LoRA {i + 1}", loaded_parameter_dict, results
        )
...
def get_str(
    key: str, fallback: str | None, source_dict: dict, results: list, default=None
):
    try:
...
def get_steps(
    ...
    try:
        ...
        if (
            h not in iter(Steps)
            or Steps(h).name.casefold()
            != source_dict.get("performance", "").replace(" ", "_").casefold()
        ):
            results.append(h)
            return
        results.append(-1)
    ...
    for settings_key, meta_key in modules.config.possible_preset_keys.items():
        if settings_key == "default_loras":
            ...
            for index, lora in enumerate(loras[:5]):
                preset_prepared[f"lora_combined_{index + 1}"] = " : ".join(
                    map(str, lora)
                )
...
class MetadataParser(ABC):
    ...
    def set_data(
        ...
        base_model_path = get_file_from_folder_list(
        ...
        if refiner_model_name not in ["", "None"]:
            ...
            refiner_model_path = get_file_from_folder_list(
                refiner_model_name, modules.config.paths_checkpoints
            )
        ...
        for lora_name, lora_weight in loras:
            if lora_name != "None":
                lora_path = get_file_from_folder_list(
                    lora_name, modules.config.paths_loras
                )
...
class A1111MetadataParser(MetadataParser):
        ...
        *lines, lastline = metadata.strip().split("\n")
        ...
        for line in lines:
            ...
            if line.startswith(f"{self.fooocus_to_a1111['negative_prompt']}:"):
                ...
                line = line[
                    len(f"{self.fooocus_to_a1111['negative_prompt']}:") :
                ].strip()
            if done_with_prompt:
                metadata_negative_prompt += (
                    "" if metadata_negative_prompt == "" else "\n"
                ) + line
            else:
                metadata_prompt += ("" if metadata_prompt == "" else "\n") + line

        found_styles, prompt, negative_prompt = extract_styles_from_prompt(
        ...
        for k, v in re_param.findall(lastline):
            try:
                ...
                if m is not None:
                    ...
                    data[
                        list(self.fooocus_to_a1111.keys())[
                            list(self.fooocus_to_a1111.values()).index(k)
                        ]
                    ] = v
            except Exception:
        ...
        if "raw_prompt" in data:
            ...
            if (
                metadata_prompt != raw_prompt
                and modules.sdxl_styles.fooocus_expansion not in found_styles
            ):
    ...
    def parse_string(self, metadata: dict) -> str:
        ...
        if modules.config.metadata_created_by != "":
            generation_params[self.fooocus_to_a1111["created_by"]] = (
                modules.config.metadata_created_by
            )
    ...
    def parse_json(self, metadata: dict) -> dict:
        ...
        for key, value in metadata.items():
            ...
            if key in ["base_model", "refiner_model"]:
                metadata[key] = self.replace_value_with_filename(
                    key, value, model_filenames
                )
            elif key.startswith("lora_combined_"):
                metadata[key] = self.replace_value_with_filename(
                    key, value, lora_filenames
                )
            else:
                continue

...
try:
    with open(
        os.path.abspath(f"./presets/default.json"), "r", encoding="utf-8"
    ) as json_file:
...
def get_path_output() -> str:
    ...
    if args_manager.args.output_path:
        print(
            f"[CONFIG] Overriding config value path_outputs with {args_manager.args.output_path}"
        )
...
default_loras = get_config_item_or_set_default(
    key="default_loras",
    default_value=[
        ["SDXL_FILM_PHOTOGRAPHY_STYLE_BetaV0.4.safetensors", 0.25],
        ["None", 1.0],
        ["None", 1.0],
    ],
...
default_max_lora_number = get_config_item_or_set_default(
    ...
    default_value=(
        len(default_loras)
        if isinstance(default_loras, list) and len(default_loras) > 0
        else 5
    ),
...
default_aspect_ratio = get_config_item_or_set_default(
    ...
    default_value=(
        "1024*1024"
        if "1024*1024" in available_aspect_ratios
...
metadata_created_by = get_config_item_or_set_default(
    key="metadata_created_by", default_value="", validator=lambda x: isinstance(x, str)
)

example_inpaint_prompts = [[x] for x in example_inpaint_prompts]

config_dict["default_loras"] = default_loras = default_loras[
    :default_max_lora_number
] + [["None", 1.0] for _ in range(default_max_lora_number - len(default_loras))]
...
if REWRITE_PRESET and isinstance(args_manager.args.preset, str):
    ...
    with open(save_path, "w", encoding="utf-8") as json_file:
        json.dump(
            {k: config_dict[k] for k in possible_preset_keys}, json_file, indent=4
        )

...
def patched_encode_token_weights(self, token_weight_pairs):
    ...
    if has_weights or sections == 0:
        to_encode.append(
            ldm_patched.modules.sd1_clip.gen_empty_tokens(
                self.special_tokens, max_token_len
            )
        )

    out, pooled = self.encode(to_encode)
    if pooled is not None:
        first_pooled = pooled[0:1].to(
    ...
    return torch.cat(output, dim=-2).to(
        ldm_patched.modules.model_management.intermediate_device()
    ), first_pooled


def patched_SDClipModel__init__(
    self,
    max_length=77,
    freeze=True,
    layer="last",
    ...
    **kwargs,
    ...
    if textmodel_json_config is None:
        textmodel_json_config = os.path.join(
            os.path.dirname(os.path.realpath(ldm_patched.modules.sd1_clip.__file__)),
...
def patched_SDClipModel_forward(self, tokens):
    ...
    if self.text_projection is not None and pooled_output is not None:
        pooled_output = (
            pooled_output.float().to(self.text_projection.device)
...
def patched_ClipVisionModel_encode_image(self, image):
    ...
    for k in outputs:
        ...
        if t is not None:
            if k == "hidden_states":
                outputs["penultimate_hidden_states"] = t[-2].to(
                    ldm_patched.modules.model_management.intermediate_device()
                )
...
def patch_all_clip():
    ldm_patched.modules.sd1_clip.ClipTokenWeightEncoder.encode_token_weights = (
    ...
    ldm_patched.modules.clip_vision.ClipVisionModel.__init__ = (
        patched_ClipVisionModel__init__
    )
    ldm_patched.modules.clip_vision.ClipVisionModel.encode_image = (
        patched_ClipVisionModel_encode_image
    )
    return

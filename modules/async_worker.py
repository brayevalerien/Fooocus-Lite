    ...
    def handler(async_task):
                ...
                if inpaint_mask_upload_checkbox:
                    if isinstance(inpaint_mask_image_upload, np.ndarray):
                        if inpaint_mask_image_upload.ndim == 3:
                            ...
                            inpaint_mask_image_upload = np.mean(
                                inpaint_mask_image_upload, axis=2
                            )
        ...
        if not skip_prompt_processing:
            ...
            for i in range(image_number):
                ...
                task_extra_negative_prompts = [
                    apply_wildcards(pmt, task_rng) for pmt in extra_negative_prompts
                ]
        ...
        if "upscale" in goals:
            ...
            if direct_return:
                ...
                yield_result(
                    async_task, uov_input_image_path, do_not_show_finished_images=True
                )
                return
        ...
        if scheduler_name == "lcm":
            ...
            if pipeline.final_unet is not None:
                pipeline.final_unet = core.opModelSamplingDiscrete.patch(
                    pipeline.final_unet, sampling="lcm", zsnr=False
                )[0]
            if pipeline.final_refiner_unet is not None:
                pipeline.final_refiner_unet = core.opModelSamplingDiscrete.patch(
                    pipeline.final_refiner_unet, sampling="lcm", zsnr=False
                )[0]

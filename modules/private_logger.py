...
def get_current_html_path(output_format=None):
    output_format = (
        output_format if output_format else modules.config.default_output_format
    )
    date_string, local_temp_filename, only_name = generate_temp_filename(
        folder=modules.config.path_outputs, extension=output_format
    )
...
def log(
    img, metadata, metadata_parser: MetadataParser | None = None, output_format=None
) -> str:
    path_outputs = (
        args_manager.args.temp_path
        if args_manager.args.disable_image_log
        else modules.config.path_outputs
    )
    output_format = (
        output_format if output_format else modules.config.default_output_format
    )
    date_string, local_temp_filename, only_name = generate_temp_filename(
        folder=path_outputs, extension=output_format
    )
    os.makedirs(os.path.dirname(local_temp_filename), exist_ok=True)

    parsed_parameters = (
        metadata_parser.parse_string(metadata.copy())
    ...
    js_txt = urllib.parse.quote(
        json.dumps({k: v for _, k, v in metadata}, indent=0), safe=""
    )
    item += (
        f"</br><button onclick=\"to_clipboard('{js_txt}')\">Copy to Clipboard</button>"
    )

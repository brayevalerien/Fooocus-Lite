...
styles_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../sdxl_styles/")
)
    ...
    try:
        with open(os.path.join(styles_path, styles_file), encoding="utf-8") as f:
            for entry in json.load(f):
                ...
                negative_prompt = (
                    entry["negative_prompt"] if "negative_prompt" in entry else ""
                )
    ...
    for _ in range(wildcards_max_bfs_depth):
        ...
        for placeholder in placeholders:
            try:
                words = (
                    open(
                        os.path.join(directory, f"{placeholder}.txt"), encoding="utf-8"
                    )
                    .read()
                    .splitlines()
                )

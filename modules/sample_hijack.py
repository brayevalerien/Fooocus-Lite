...
from ldm_patched.modules.sample import (
...
def clip_separate_inner(c, p, target_model=None, target_clip=None):
    if target_model is None or isinstance(target_model, SDXLRefiner):
        ...
        final_layer_norm = (
            target_clip.cond_stage_model.clip_l.transformer.text_model.final_layer_norm
        )
        ...
        final_layer_norm.to(
            device=final_layer_norm_origin_device, dtype=final_layer_norm_origin_dtype
        )
...
def clip_separate(cond, target_model=None, target_clip=None):
    ...
    for c, px in cond:
        ...
        c, p = clip_separate_inner(
            c, p, target_model=target_model, target_clip=target_clip
        )
...
def sample_hacked(
    ...
    pre_run_control(
        model, positive
    )  # negative is not necessary in Fooocus, 0.5s faster.
    ...
    if current_refiner is not None and hasattr(current_refiner.model, "extra_conds"):
        positive_refiner = clip_separate_after_preparation(
            positive, target_model=current_refiner.model
        )
        negative_refiner = clip_separate_after_preparation(
            negative, target_model=current_refiner.model
        )

        positive_refiner = encode_model_conds(
            current_refiner.model.extra_conds,
            positive_refiner,
            noise,
            device,
            "positive",
        ...
        negative_refiner = encode_model_conds(
            current_refiner.model.extra_conds,
            negative_refiner,
            noise,
            device,
    ...
    def refiner_switch():
        ...
        extra_args["model_options"] = {
            k: {} if k == "transformer_options" else v
            for k, v in extra_args["model_options"].items()
        }

        models, inference_memory = get_additional_models(
            positive_refiner, negative_refiner, current_refiner.model_dtype()
        )
        ldm_patched.modules.model_management.load_models_gpu(
            [current_refiner] + models,
            model.memory_required([noise.shape[0] * 2] + list(noise.shape[1:]))
            + inference_memory,
        )
...
def calculate_sigmas_scheduler_hacked(model, scheduler_name, steps):
    if scheduler_name == "karras":
        ...
        sigmas = SDTurboScheduler().get_sigmas(
            namedtuple("Patcher", ["model"])(model=model), steps=steps, denoise=1.0
        )[0]
    else:
...
ldm_patched.modules.samplers.calculate_sigmas_scheduler = (
    calculate_sigmas_scheduler_hacked
)

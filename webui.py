    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            with gr.Row(elem_classes='type_row'):
                with gr.Column(scale=17):
                    prompt = gr.Textbox(show_label=False, placeholder="Positive prompt (what you want to see) goes here.", elem_id='positive_prompt',
                                        container=False, autofocus=True, elem_classes='type_row', lines=3)
                ...
                with gr.Column(scale=3, min_width=0):
                    ...
                    skip_button = gr.Button(
                        label="Skip", value="Skip", elem_classes='type_row_half', visible=False)
            ...
            negative_prompt = gr.Textbox(show_label=False, placeholder="Negative prompt (what you DO NOT want to see) goes here.", elem_id='negative_prompt',
                                         container=False, autofocus=False, elem_classes='type_row', lines=3)

            with gr.Row(elem_classes='advanced_check_row'):
                input_image_checkbox = gr.Checkbox(
                    label='Input Image', value=True, container=False, elem_classes='min_check', visible=False)
                advanced_checkbox = gr.Checkbox(
                    label='Advanced', value=modules.config.default_advanced_checkbox, container=False, elem_classes='min_check', visible=False)

            with gr.Tabs(visible=True):
                with gr.TabItem(label='Guide images', visible=False) as ip_tab:
                    with gr.Row(visible=True) as image_input_panel:
                            ...
                            for _ in range(flags.controlnet_image_count):
                                with gr.Column():
                                    ...
                                    with gr.Column(visible=True) as ad_col:
                                        with gr.Row():
                                            default_end, default_weight = flags.default_parameters[
                                                flags.default_ip]

                                            # Stop at value
                                            ip_stop = gr.Slider(
                                                label='Strength', minimum=0.0, maximum=1.0, step=0.001, value=default_end, visible=True)
                                            ip_stops.append(ip_stop)
                                            ip_ctrls.append(ip_stop)

                                            # Weight is invisible in Lite, set default values in the flags.default_parameters dict
                                            ip_weight = gr.Slider(
                                                label='Weight', minimum=0.0, maximum=2.0, step=0.001, value=default_weight, visible=False)
                                            ip_weights.append(ip_weight)
                                            ip_ctrls.append(ip_weight)

                                        ip_type = gr.Radio(
                                            label='Type', choices=flags.ip_list, value=flags.default_ip, container=False)
                        ...
                        ip_advanced = gr.Checkbox(
                            label='Advanced', value=True, container=False, visible=False)
                ...
                with gr.TabItem(label='Inpaint or Outpaint') as inpaint_tab:
                    with gr.Row():
                        inpaint_input_image = grh.Image(label='Drag inpaint or outpaint image to here', show_label=False, source='upload',
                                                        type='numpy', tool='sketch', height=500, brush_color="#FFFFFF", elem_id='inpaint_canvas')
                        inpaint_mask_image = grh.Image(
                            label='Mask Upload', source='upload', type='numpy', height=500, visible=False)

                    with gr.Row():
                        inpaint_additional_prompt = gr.Textbox(
                            placeholder="Additional Prompt.", elem_id='inpaint_additional_prompt', label='Inpaint Additional Prompt', visible=False)
                        outpaint_selections = gr.CheckboxGroup(
                            choices=['Left', 'Right', 'Top', 'Bottom'], value=[], label='Outpaint Direction')
                        inpaint_mode = gr.Dropdown(
                            choices=modules.flags.inpaint_options, value=modules.flags.inpaint_option_default, label='Mode')
                    example_inpaint_prompts = gr.Dataset(samples=modules.config.example_inpaint_prompts, label='Additional Prompt Quick List', components=[
                        inpaint_additional_prompt], visible=False)
                    # gr.HTML(
                        ...
                        with gr.Row():
                            with gr.Column():
                                uov_input_image = grh.Image(
                                    label='Drag above image to here', source='upload', type='numpy')
                            with gr.Column():
                                uov_method = gr.Radio(
                                    label='Upscale or Variation:', choices=flags.uov_list, value=flags.disabled)
                                gr.HTML(
                                    '<a href="https://github.com/lllyasviel/Fooocus/discussions/390" target="_blank">\U0001F4D4 Document</a>')
                    with gr.TabItem(label='Describe') as desc_tab:
                        with gr.Row():
                            with gr.Column():
                                desc_input_image = grh.Image(
                                    label='Drag any image to here', source='upload', type='numpy')
                            with gr.Column():
                                desc_method = gr.Radio(
                                    label='Content Type',
                                    choices=[flags.desc_type_photo,
                                             flags.desc_type_anime],
                    ...
                    with gr.TabItem(label='Metadata') as load_tab:
                        with gr.Column():
                            metadata_input_image = grh.Image(
                                label='Drag any image generated by Fooocus here', source='upload', type='filepath')
            ...
            ip_tab.select(lambda: 'ip', outputs=current_tab,
                          queue=False, _js=down_js, show_progress=False)
            desc_tab.select(lambda: 'desc', outputs=current_tab,
                            queue=False, _js=down_js, show_progress=False)
        ...
        with gr.Column(scale=1, visible=modules.config.default_advanced_checkbox) as advanced_column:
            ...
            image_number = gr.Slider(
                label='Image Count', minimum=1, maximum=modules.config.default_max_image_number, step=1, value=modules.config.default_image_number)
            ...
            def update_history_link():
                ...
                return gr.update(value=f'<a href="file={get_current_html_path(output_format)}" target="_blank">See previously generated images parameters</a>')

            history_link = gr.HTML(visible=False)
            shared.gradio_root.load(
                update_history_link, outputs=history_link, queue=False, show_progress=False)
            
            # Hide unecessary settings tabs
            with gr.Tabs(visible=False):
                ...
                with gr.Tab(label='Advanced'):
                    ...
                    with gr.Column(visible=False) as dev_tools:
                        ...
                        with gr.Tab(label='Inpaint'):
                            ...
                            inpaint_mask_upload_checkbox = gr.Checkbox(
                                label='Enable Mask Upload', value=False)
                    ...
                    def model_refresh_clicked():
                        ...
                        for i in range(modules.config.default_max_lora_number):
                            results += [gr.update(interactive=True), gr.update(
                                choices=['None'] + modules.config.lora_filenames), gr.update()]
                        return results
        ...
        performance_selection.change(lambda x: [gr.update(interactive=x != 'Extreme Speed')] * 11 +
                                               ...
                                               [gr.update(
                                                   interactive=x != 'Extreme Speed', value=x == 'Extreme Speed', )] * 1,
        ...
        load_parameter_button.click(modules.meta_parser.load_parameter_button_click, inputs=[
                                    prompt, state_is_generating], outputs=load_data_outputs, queue=False, show_progress=False)

...
shared.gradio_root = gr.Blocks(
    theme=args_manager.args.theme_name, title=title, css=modules.html.css
).queue()
...
with shared.gradio_root:
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            with gr.Row(elem_classes="type_row"):
                with gr.Column(scale=17):
                    prompt = gr.Textbox(
                        show_label=False,
                        placeholder="Positive prompt (what you want to see) goes here.",
            ...
            negative_prompt = gr.Textbox(
                show_label=False,
                placeholder="Negative prompt (what you DO NOT want to see) goes here.",
            ...
            with gr.Tabs(visible=True):
                ...
                with gr.Tabs(visible=False):
                    with gr.TabItem(
                        label="Upscale or Variation", visible=False
                    ) as uov_tab:
                        with gr.Row():
                            with gr.Column():
                                uov_input_image = grh.Image(
                                    label="Drag above image to here",
                                    source="upload",
                                    type="numpy",
                                )
                            with gr.Column():
                    ...
                    with gr.TabItem(label="Metadata") as load_tab:
                        with gr.Column():
                            metadata_input_image = grh.Image(
                                label="Drag any image generated by Fooocus here",
                                source="upload",
                                type="filepath",
                            )
                        ...
                        def trigger_metadata_preview(filepath):
                            parameters, metadata_scheme = (
                                modules.meta_parser.read_info_from_image(filepath)
                            )
            ...
            ip_advanced.change(
                lambda: None, queue=False, show_progress=False, _js=down_js
            )
        ...
        with gr.Column(
            scale=1, visible=modules.config.default_advanced_checkbox
        ) as advanced_column:
            ...
            mixing_image_prompt_and_inpaint = gr.Checkbox(
                label="Use guide images for inpainting", value=False
            )
            ...
            with gr.Accordion(
                label="LoRAs", visible=args_manager.args.show_loras, open=False
            ):
                with gr.Group():
                    lora_ctrls = []
            ...
            def update_history_link():
                ...
                return gr.update(
                    value=f'<a href="file={get_current_html_path(output_format)}" target="_blank">See previously generated images parameters</a>'
                )
            ...
            with gr.Tabs(visible=False):
                ...
                with gr.Tab(label="Advanced"):
                    ...
                    with gr.Column(visible=False) as dev_tools:
                        with gr.Tab(label="Debug Tools"):
                            ...
                            disable_intermediate_results = gr.Checkbox(
                                ...
                                value=modules.config.default_performance
                                == "Extreme Speed",
                                interactive=modules.config.default_performance
                                != "Extreme Speed",
                        ...
                        with gr.Tab(label="Control"):
                            debugging_cn_preprocessor = gr.Checkbox(
                                label="Debug Preprocessors",
                                value=False,
                                info="See the results from preprocessors.",
                            )
                            skipping_cn_preprocessor = gr.Checkbox(
                                label="Skip Preprocessors",
                                value=False,
                        ...
                        with gr.Tab(label="Inpaint"):
                            ...
                            inpaint_mask_upload_checkbox.change(
                                lambda x: gr.update(visible=x),
                    ...
                    model_refresh.click(
                        model_refresh_clicked,
                        [],
                        [base_model, refiner_model] + lora_ctrls,
                        queue=False,
                        show_progress=False,
                    )

        performance_selection.change(
            lambda x: [gr.update(interactive=x != "Extreme Speed")] * 11
            + [gr.update(visible=x != "Extreme Speed")] * 1
            + [
                gr.update(
                    interactive=x != "Extreme Speed",
                    value=x == "Extreme Speed",
                )
            ]
            * 1,
        ...
        def trigger_metadata_import(filepath, state_is_generating):
            ...
            return modules.meta_parser.load_parameter_button_click(
                parsed_parameters, state_is_generating
            )

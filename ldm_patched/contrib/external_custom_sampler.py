...
class BasicScheduler:
    ...
    def get_sigmas(self, model, scheduler, steps, denoise):
        ...
        sigmas = ldm_patched.modules.samplers.calculate_sigmas_scheduler(
            model.model, scheduler, total_steps
        ).cpu()
...
class SamplerDPMPP_2M_SDE:
    ...
    def get_sampler(self, solver_type, eta, s_noise, noise_device):
        ...
        sampler = ldm_patched.modules.samplers.ksampler(
...
class SamplerCustom:
    ...
    def sample(
        ...
        x0_output = {}
        callback = ldm_patched.utils.latent_visualization.prepare_callback(
            model, sigmas.shape[-1] - 1, x0_output
        )

        disable_pbar = not ldm_patched.modules.utils.PROGRESS_BAR_ENABLED
        samples = ldm_patched.modules.sample.sample_custom(
            model,
            noise,
            cfg,
            sampler,

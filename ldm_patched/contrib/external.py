...
class SaveLatent:
    ...
    def save(
        self, samples, filename_prefix="ldm_patched", prompt=None, extra_pnginfo=None
    ):
        full_output_folder, filename, counter, subfolder, filename_prefix = (
            ldm_patched.utils.path_utils.get_save_image_path(
...
class CheckpointLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "config_name": (
                    ldm_patched.utils.path_utils.get_filename_list("configs"),
                ),
                "ckpt_name": (
                    ldm_patched.utils.path_utils.get_filename_list("checkpoints"),
                ),
            }
        }
    ...
    def load_checkpoint(
        ...
        return ldm_patched.modules.sd.load_checkpoint(
            config_path,
            ckpt_path,
            output_vae=True,
            output_clip=True,
            embedding_directory=ldm_patched.utils.path_utils.get_folder_paths(
                "embeddings"
            ),
        )
...
class CheckpointLoaderSimple:
    ...
    def load_checkpoint(self, ckpt_name, output_vae=True, output_clip=True):
        ckpt_path = ldm_patched.utils.path_utils.get_full_path("checkpoints", ckpt_name)
        out = ldm_patched.modules.sd.load_checkpoint_guess_config(
            ckpt_path,
            output_vae=True,
            output_clip=True,
            embedding_directory=ldm_patched.utils.path_utils.get_folder_paths(
...
class DiffusersLoader:
    ...
    def load_checkpoint(self, model_path, output_vae=True, output_clip=True):
        ...
        return ldm_patched.modules.diffusers_load.load_diffusers(
            model_path,
...
class LoraLoader:
    ...
    def load_lora(self, model, clip, lora_name, strength_model, strength_clip):
        ...
        model_lora, clip_lora = ldm_patched.modules.sd.load_lora_for_models(
...
class VAELoader:
    ...
    def load_taesd(name):
        ...
        enc = ldm_patched.modules.utils.load_torch_file(
            ldm_patched.utils.path_utils.get_full_path("vae_approx", encoder)
        )
        for k in enc:
            sd["taesd_encoder.{}".format(k)] = enc[k]

        dec = ldm_patched.modules.utils.load_torch_file(
            ldm_patched.utils.path_utils.get_full_path("vae_approx", decoder)
        )
        for k in dec:
...
class ControlNetLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "control_net_name": (
                    ldm_patched.utils.path_utils.get_filename_list("controlnet"),
                )
            }
        }
    ...
    def load_controlnet(self, control_net_name):
        controlnet_path = ldm_patched.utils.path_utils.get_full_path(
...
class DiffControlNetLoader:
    ...
    def load_controlnet(self, model, control_net_name):
        ...
        controlnet = ldm_patched.modules.controlnet.load_controlnet(
...
class UNETLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "unet_name": (ldm_patched.utils.path_utils.get_filename_list("unet"),),
            }
        }
...
class CLIPLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "clip_name": (ldm_patched.utils.path_utils.get_filename_list("clip"),),
            }
        }
    ...
    def load_clip(self, clip_name):
        ...
        clip = ldm_patched.modules.sd.load_clip(
...
class DualCLIPLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "clip_name1": (ldm_patched.utils.path_utils.get_filename_list("clip"),),
                "clip_name2": (ldm_patched.utils.path_utils.get_filename_list("clip"),),
            }
        }
...
class CLIPVisionLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "clip_name": (
                    ldm_patched.utils.path_utils.get_filename_list("clip_vision"),
                ),
            }
        }
...
class StyleModelLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "style_model_name": (
                    ldm_patched.utils.path_utils.get_filename_list("style_models"),
                )
            }
        }
    ...
    def load_style_model(self, style_model_name):
        style_model_path = ldm_patched.utils.path_utils.get_full_path(
...
class GLIGENLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "gligen_name": (
                    ldm_patched.utils.path_utils.get_filename_list("gligen"),
                )
            }
        }
...
class LatentUpscale:
    ...
    def upscale(self, samples, upscale_method, width, height, crop):
        if width == 0 and height == 0:
            ...
            s["samples"] = ldm_patched.modules.utils.common_upscale(
...
class LatentBlend:
    ...
    def blend(
        ...
        if samples1.shape != samples2.shape:
            ...
            samples2 = ldm_patched.modules.utils.common_upscale(
...
class SaveImage:
    ...
    def save_images(
        self, images, filename_prefix="ldm_patched", prompt=None, extra_pnginfo=None
    ):
...
class ImageScale:
    ...
    def upscale(self, image, upscale_method, width, height, crop):
        if width == 0 and height == 0:
            ...
            s = ldm_patched.modules.utils.common_upscale(
...
class ImageBatch:
    ...
    def batch(self, image1, image2):
        if image1.shape[1:] != image2.shape[1:]:
            image2 = ldm_patched.modules.utils.common_upscale(
                image2.movedim(-1, 1),
                image1.shape[2],
                image1.shape[1],
                "bilinear",
...
def init_custom_nodes():
    extras_dir = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "ldm_patched_extras"
    )
    extras_files = [
        "nodes_latent.py",

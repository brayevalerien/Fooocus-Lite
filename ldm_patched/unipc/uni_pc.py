...
class UniPC:
    ...
    def data_prediction_fn(self, x, t):
        ...
        if self.thresholding:
            ...
            x0 = torch.clamp(x0, -s, s) / s
        if self.noise_mask is not None:
            x0 = x0 * self.noise_mask + (1.0 - self.noise_mask) * self.masked_image
        return x0
    ...
    def sample(
        ...
        if method == "multistep":
            ...
            for step_index in trange(steps, disable=disable_pbar):
                if self.noise_mask is not None:
                    x = x * self.noise_mask + (1.0 - self.noise_mask) * (
                        self.masked_image
                        * self.noise_schedule.marginal_alpha(timesteps[step_index])
                        + self.noise
                        * self.noise_schedule.marginal_std(timesteps[step_index])
                    )
                if step_index == 0:

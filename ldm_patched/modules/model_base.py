...
class BaseModel(torch.nn.Module):
    ...
    def memory_required(self, input_shape):
        if (
            ...
            return (
                area * ldm_patched.modules.model_management.dtype_size(dtype) / 50
            ) * (1024 * 1024)
        else:
...
class SVD_img2vid(BaseModel):
    ...
    def extra_conds(self, **kwargs):
        ...
        if "time_conditioning" in kwargs:
            out["time_context"] = ldm_patched.modules.conds.CONDCrossAttn(
                kwargs["time_conditioning"]
            )

        out["image_only_indicator"] = ldm_patched.modules.conds.CONDConstant(
...
class Stable_Zero123(BaseModel):
    def __init__(
        ...
        super().__init__(model_config, model_type, device=device)
        self.cc_projection = ldm_patched.modules.ops.manual_cast.Linear(
